...
import torch
from model import resnet_34,VGG_19
...
model_res34 = resnet_34().to(device)
#model_vgg = VGG_16().to(device)
print(model_res34)

#define train and test function
loss_fn = nn.CrossEntropyLoss()
optimizer = torch.optim.Adam(model_res34.parameters(), lr=1e-3)
...
test(test_dataloader, model_res34, loss_fn)
test(train_dataloader, model_res34, loss_fn)
for t in range(epochs):
    print(f"Epoch {t+1}\n-------------------------------")
    train(train_dataloader, model_res34, loss_fn, optimizer)
    test(test_dataloader, model_res34, loss_fn)
    test(train_dataloader, model_res34, loss_fn)
    torch.save(model_res34,'./weights/res34/{}.pth'.format(t))
print("Done!")
np.save("./loss/res34.npy",plot_loss)